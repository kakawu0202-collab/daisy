/**
 * Yumin K1 Portal — Shared PWA Logic
...
(function() {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/sw.js')
      .then(function(reg) {
        console.log('[Yumin PWA] SW registered:', reg.scope);
        ...
        reg.addEventListener('updatefound', function() {
          ...
          newWorker.addEventListener('statechange', function() {
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
              console.log('[Yumin PWA] New version available — will apply on reload');
            }
          });
        });
      ...
      .catch(function(err) {
        console.warn('[Yumin PWA] SW registration failed:', err);
      });
  }
  ...
  function subscribePush(reg) {
    ...
    reg.pushManager.getSubscription().then(function(sub) {
      ...
      // Auto-subscribe if browser supports it
      if (Notification.permission === 'granted') {
        reg.pushManager.subscribe({
          userVisibleOnly: true,
          applicationServerKey: urlBase64ToUint8Array(
            'BEl62iGhoA4jOJ429JEqYHZMrLFHNiYP_NkcIsWgZhUPfFkyFUsfQErNxfUevHlckRzXHllMdlDxGZRJbvBLkwI'
          )
  ...
  window.addEventListener('appinstalled', function() {
    console.log('[Yumin PWA] App installed');
    deferredPrompt = null;
  });
