...
PORT = int(os.environ.get('SCOS_CONTROL_PORT', '8900'))  # dev: 8901
ENGINE_PORT = int(os.environ.get('SCOS_ENGINE_PORT', '8700'))  # dev: 8701
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path in ('/', '/index.html'):
            self._send_html(CONTROL_HTML.replace('localhost:8700', f'localhost:{ENGINE_PORT}'))
    ...
    def do_POST(self):
        ...
        if path == 'start':
            ...
            try:
                ...
                resp = _r.post(f'http://localhost:{ENGINE_PORT}/trigger', timeout=10)
    ...
    def _status(self):
        ...
        try:
            import requests as _r
            es = _r.get(f'http://localhost:{ENGINE_PORT}/status', timeout=3).json()
